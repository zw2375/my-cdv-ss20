...
function averageData(data){
  let newData = [];
  let keys = Object.keys(data[ data.length-1 ]); //Making keys = the poperty names of the last key in the object
  for(let i = 0; i < keys.length; i++){
    let key = keys[i]; // Making key = each poperty in keys
    let sum = 0;
    let num = 0;
    ...
    let avg = sum/num;
    if(!isNaN(avg)){ //make sure it's number
